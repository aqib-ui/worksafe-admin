...
const AdvanceSearch = ({ advanceModalSearchModal, handleCancel }) => {
    ...
    const [allComp, setAllComp] = useState([{ id: totalCount ?? 0, value: "All" }, { id: allResult?.assets?.length ?? 0, value: "Assets" }, { id: allResult?.alerts?.length ?? 0, value: "Alerts" }, { id: allResult?.pois?.length ?? 0, value: "POIs" }, { id: allResult?.projects?.length ?? 0, value: "Projects" }, { id: allResult?.workorders?.length ?? 0, value: "Work Orders" },])
    ...
    const LoadSearch = async ({ Query }) => {
        ...
        const bodyData = {
            ...
            radius: selectedPosition !== null ? radius : undefined,
            query: Query || "",
            page: page || undefined,
            from_dashboard: false,
            created_by_me: isCreatedByMe || undefined,
            polygon_type: polygonType?.length > 0 ? polygonType : undefined
        };
        ...
        if (response.status == 200 || response.status == 201) {
            ...
            const totalCount2 =
                ...
                (res?.assets?.length ?? 0);
            setAllComp([{ id: totalCount2 ?? 0, value: "All" }, { id: res?.assets?.length ?? 0, value: "Assets" }, { id: res?.alerts?.length ?? 0, value: "Alerts" }, { id: res?.pois?.length ?? 0, value: "POIs" }, { id: res?.projects?.length ?? 0, value: "Projects" }, { id: res?.workorders?.length ?? 0, value: "Work Orders" },])
            setAllResult(res)
            ...
            >
                                        ...
                                        <div className={Style.PosterMane} onClick={() => item.type === "asset" ? viewAssets(item?._id) : item.type == "project" ? viewProject(item._id) : item.type == "workorder" ? viewWorkOrder(item?._id) : item.type == "poi" ? viewPOIs(item?._id) : item.type == "alert" ? viewAlerts(item?._id) : null}>
                                            <div >
                                                ...
                                                {item.type === "project" || item.type === "asset" ?
                                                    ...
                                                    : item.type === "workorder" ?
                                                        <Tag style={{ color: item?.priority == "Immediate" ? "#C94040" : item?.priority == "High" ? "#C9A240" : item?.priority == "Standard" ? '#1C8F5D' : null }} color={item?.priority == "Immediate" ? "#F14D4D14" : item?.priority == "High" ? "#F1C34D14" : item?.priority == "Standard" ? '#4DF15E14' : null}>
                                                            {item?.priority}
                                                        </Tag>
                                                        :
                                                        <Tag style={{ color: item?.riskLevel == "No Threat" ? "#666d80" : item?.riskLevel == "Lowest" ? "#17736E" : item?.riskLevel == "Moderate" ? '#926E26' : item?.riskLevel == "High" ? '#D32029' : item?.riskLevel == "Extreme" ? '#7F1319' : null }} color={item?.riskLevel == "No Threat" ? "rgba(102, 109, 128,0.1)" : item?.riskLevel == "Lowest" ? "rgba(23, 115, 110,0.1)" : item?.riskLevel == "Moderate" ? 'rgba(146, 110, 38,0.1)' : item?.riskLevel == "High" ? 'rgba(211, 32, 41,0.1)' : item?.riskLevel == "Extreme" ? 'rgba(127, 19, 25,0.1)' : null}>
                ...
                {latestTab.length > 0 &&
                    <div className={Style.PaginationQuery}>
                        <button style={{ cursor: page <= 1 || searchGlobeLoad ? "no-drop" : "pointer" }} onClick={() => setPage(page <= 1 ? 1 : page - 1)} disabled={searchGlobeLoad}><MdKeyboardArrowLeft size={25} /></button>
                        <div className={Style.OnPageCC}>{page}</div>
                        <button style={{ cursor: checkPage() || searchGlobeLoad ? "no-drop" : "pointer" }} disabled={checkPage() || searchGlobeLoad} onClick={() => setPage(page + 1)}><MdOutlineKeyboardArrowRight size={25} /></button>
                    </div>
                }
            </Modal>
