...
import Style from './MusterStationCreateScreen.module.css'
...
const MusterStationCreateScreen = ({ GetWorkSiteByID, GetWorkSite, GetRoles, WorksiteReducer, GetManagerInWorksite, GetTeamInWorksite, TeamReducer, AlertsReducer, PoiReducer, GetAllWorkOrderUnLink, InserTeamUserV2 }) => {
    ...
    const readId = queryParams.get("isRead");
    const fromEdit = queryParams.get("fromEdit");
    const fromView = queryParams.get("fromView");
    const currentWorksiteLoaded = localStorage.getItem(fromEdit == "true" ? "Qe!47Xw+Tz@p9KbL#3yVNr" : "Bm_8Xr#Q+21fGt!zY@Hj6Lp")
    ...
    const { worksiteDetail } = WorksiteReducer
    ...
    `,
            disableAutoPan: true,
            headerDisabled: true,
            pixelOffset: new window.google.maps.Size(0, 10), // position InfoWindow above the icon
        });
    ...
    const drawCircleWorkSite = (loc, radius, safetyZone) => {
    ...
    useEffect(() => {
        ...
        if (savedForms1 && typeof savedForms1 === 'object') {
            const polygons = JSON.parse(savedForms1?.polygon || "{}");
            ...
            if (polygons?.type == "Polygon") {
                ...
                setPadding(polygons?.safetyZone)
            }
            if (polygons?.type == "Polyline") {
                ...
                setSafetyOffsetMore(polygons?.safetyZone)
            }
        }
    }, [])
    ...
    useEffect(() => {
        if (fromEdit) {
            if (worksiteDetail?._doc) {
                const firstLocation = worksiteDetail?._doc?.polygon?.locations?.[0];
                const polygons = worksiteDetail?._doc?.polygon
                if (polygons?.type == "Circle") {
                    const killtime = setTimeout(() => {
                        drawCircleWorkSite({
                            lat: Number(firstLocation[0]),
    ...
    const drawCircleForSee = (a, b, c) => {
        ...
        const parent = new window.google.maps.Circle({
            ...
            editable: readId === "true" ? false : true,
            zIndex: 999
        });
        ...
        if (
            ...
            try {
                ...
                if (queryId) {
                    ...
                    const savedForms = JSON.parse(localStorage.getItem(key)) || [];
                    const newForms = savedForms.map(form =>
                        form.id === queryId ? { ...form, ...payload } : form
                    );
                    localStorage.setItem(key, JSON.stringify(newForms));
                    window.location.href = '/worksite/create'
                }
                else {
                    if (fromEdit == "true") {
                        const payload = {
                    ...
                    <div className={Style.SecondaryHeader}>
                        ...
                        {readId === "true" ? (
                            ...
                            <div>
                                ...
                                >
                                    {CreateLoading
                                        ? `${queryId ? "Saving" : "Adding"} Muster Station...`
                                        : `${queryId ? "Save" : "Add"} Muster Station`}
                                </button>
                            </div>
                        )}
                            ...
                            <div className={Style.FeildColRight}>
                                ...
                                <Controller
                                    ...
                                    render={({ field: { onChange, value } }) => (
                                        <Input disabled={queryId && readId == "true" ? true : false} onChange={onChange} value={value} status={errors?.title?.message !== undefined ? 'error' : ''} placeholder='Enter Title' style={{ height: 45, marginTop: 3 }} />
                                    )}
                                    name="title"
                                />
                            </div>
