...
import { TASK_GET_ALERTS_COMPLETE, TASK_GET_ARCHIVED_ALERTS_COMPLETE, TASK_GET_POI_ARCHIVED_COMPLETE, TASK_GET_POI_COMPLETE } from "../../../../store/actions/types";


const DEFAULT_EXTRA_DATA = { name: '', type: '' };


const AlertFilter = ({ setPage, setIsNext, loading, GetPOI, workSite, page, searchQuery, setParamsNew }) => {
    ...
    const AddAlertLevelTypeKey = (UpperParams) => {
        setAlertLevelFilter((prev) => {
            if (prev.includes(UpperParams)) {
                return prev.filter(item => item !== UpperParams);
            } else {
                return [...prev, UpperParams];
                ...
                <>
                    <Row >
                        ...
                        <Col xxl={24} xl={24} lg={24} md={24} sm={24} xs={24}>
                            <div className={Style.FeildCol} style={{ marginTop: 16 }}>
                                <div className={Style.ResetterIcon}>
                                    <label className={Style.LabelFilter}>Select Alert Type</label>
                                    <label onClick={() => loading ? null : setAlertLevelFilter([])} className={Style.ResetButton}>Reset</label>
                                </div>
                                <div>
                                    <Checkbox disabled={loading} onClick={() => AddAlertLevelTypeKey("HAZARD")} checked={alertLevelFilter.includes('HAZARD') ? true : false} defaultChecked={false}>Potential hazard</Checkbox>
                                    <Checkbox disabled={loading} onClick={() => AddAlertLevelTypeKey("WEATHER")} checked={alertLevelFilter.includes('WEATHER') ? true : false} defaultChecked={false}>Weather Condition</Checkbox>
                                    <Checkbox disabled={loading} onClick={() => AddAlertLevelTypeKey("COMMUNICATION")} checked={alertLevelFilter.includes('COMMUNICATION') ? true : false} defaultChecked={false}>Communication</Checkbox>
                                    <Checkbox disabled={loading} onClick={() => AddAlertLevelTypeKey("SECURITY")} checked={alertLevelFilter.includes('SECURITY') ? true : false} defaultChecked={false}>Security</Checkbox>
                                </div>
                            </div>
                        </Col>
