...
// import Style from './AlertsCreateScreen.module.css'
...
// const AlertScreenCreate = ({ AlertsReducer, PoiReducer, GetAllWorkOrderUnLink }) => {
...
//     useEffect(() => {
//         const savedForms1 = JSON.parse(localStorage.getItem('v@XZ+L4mA1#cK8^y6JtEdr9')) || [];
...
//     });
//     const localData = JSON.parse(localStorage.getItem("#B9wNc@z8P^mA27!q+JDkLrX") || "{}");
...
//         },
//     });
//     const TypeOfAlertLive = watch('typeOfAlert')
//     useEffect(() => {
//         const subscription = watch((value) => {
//             const savedForms = JSON.parse(localStorage.getItem('#B9wNc@z8P^mA27!q+JDkLrX')) || [];
//             const ParseDataBefore = { ...savedForms, ...value }
//             localStorage.setItem("#B9wNc@z8P^mA27!q+JDkLrX", JSON.stringify(ParseDataBefore));
//         });
...
//         },
//     ];
//     const [extraDataList, setExtraDataList] = useState(JSON.parse(localStorage.getItem("#B9wNc@z8P^mA27!q+JDkLrX") || "{}").extraData || []);
//     const itemsEdit = [
//         {
//             label: (
//                 <div onClick={() => {
...
//                 });
//             }
//             const PrevJsonData = JSON.parse(localStorage.getItem("#B9wNc@z8P^mA27!q+JDkLrX") || "{}")
//             const UpdateJsonParr = { extraData: [...extraDataList, extraDataState], ...PrevJsonData }
//             localStorage.setItem("#B9wNc@z8P^mA27!q+JDkLrX", JSON.stringify(UpdateJsonParr));
//         }
//     };
...
//             localStorage.setItem("#B9wNc@z8P^mA27!q+JDkLrX", JSON.stringify(UpdateJsonParT));
...
//             const payloadNew = {
//                 ...body,
//                 polygon: polygon?.polygon ? JSON.stringify(polygon?.polygon) : "{}",
//             };
//             try {
...
//         localStorage.setItem('v@XZ+L4mA1#cK8^y6JtEdr9', JSON.stringify(jsonData));
//         return true
//     }
...
//                         <h1>Create Alert</h1>
//                         <div>
//                             <button style={{ cursor: "pointer" }} disabled={CreateLoading} onClick={handleSubmit(adddataWorkOrder)}>{CreateLoading ? "Adding Alert..." : "Add Alert"}</button>
//                         </div>
//                     </div>
...
// function mapStateToProps({ PoiReducer, AlertsReducer }) {
//     return { PoiReducer, AlertsReducer };
// }
// export default connect(
//     mapStateToProps,
//     { ...ProjectAction, ...POIAction }
// )(AlertScreenCreate);
...
const AlertScreenCreate = ({ GetAllWorkOrderFilterLink, GetAlertsByID, AlertsReducer, PoiReducer, GetAllWorkOrderUnLink, WorkOrderReducer }) => {
    dayjs.extend(customParseFormat);
    const [messageApi, contextHolder] = message.useMessage();
    const navigate = useNavigate();
    ...
    const currentWorkSite = localStorage.getItem("+AOQ^%^f0Gn4frTqztZadLrKg==")
    const localStoreKey = "Q9#vL2@X!mT8_Pz7&KcR*w=="
    ...
    const getStepThreeData = async () => {
        ...
        if (stepThreeData) {
            ...
            try {
                ...
                const AwsUpload = await AWSUploadModule({ messageApi, fileArray, moduleName: "report", actualFile: [...allFormDataFine?.warrantyDocumentation, ...allFormDataFine?.safetyDocumentation,] })
                const payload = {
                    title: allFormDataFine?.Title,
                    description: allFormDataFine?.description,
                    riskLevel: allFormDataFine?.threatLevel,
                    alertType: allFormDataFine?.typeOfAlerts,
                    worksiteId: currentWorkSite,
                    files: !AwsUpload ? [] : AwsUpload,
                    ...(editId && {
                        alertId: editId,
                ...
                const options = {
                    ...
                    body: JSON.stringify(editId ? [
                        ...allFormDataFine?.safetyDocumentation,
                        ...allFormDataFine?.warrantyDocumentation,
                    ] ? { ...payload, filesId: JSON.stringify(alertDetail?.files?.filter(data => !allFormDataFine?.deletePhoto.includes(data?._id)).map(data => { return data._id })) } : "" : payload),
                    signal: controller.signal,
                };
                const response = await fetch(`${baseUrl}/alerts`, options);
                ...
                if (response.status == 200 || response.status == 201) {
                    ...
                    messageApi.open({
                        type: "success",
                        content: `Alert has been ${editId ? "updated" : "created"}.`,
                    });
                    setCreateLoading(false)
                    setTimeout(() => {
                ...
                <div>
                    <div className={Style.SecondaryHeader}>
                        <div className={Style.Allpath}>
                            <h6>Alert</h6>
                            <img src={rightIcon} />
                            <h6 className={Style.activePage}>{editId ? "Edit" : "Create"} Alert</h6>
                        </div>
                        <h3>{editId ? "Edit" : "Create"} Alert</h3>
                    </div>
                </div>
                <div className={Style.TableSection}>
                    ...
                    {currentSteps === 0 && <BasicInformation PoiReducer={PoiReducer} counter={counter} workOrderGetByIDData={alertDetail} editId={editId} isValidBtn={setIsFirstStepValid} WorkOrderReducer={WorkOrderReducer} messageApi={messageApi} basicInfoSectionRef={basicInfoSectionRef} ComapnyUserData={ComapnyUserData} WorkOrderReducerData={WorkOrderReducer?.workSiteData} />}
                    {currentSteps === 1 && <POICustomization fineEdit={fineRawDrafts?._id} isDraft={isDraftWorkOrder} setIsDraft={setIsDraftWorkOrder} getStepThreeData={getStepThreeData} workOrderLinkData={workOrderLinkData} PoiReducer={PoiReducer} counter={counter} workOrderGetByIDData={alertDetail} editId={editId} messageApi={messageApi} taskAndLocationRef={taskAndLocationRef} createLoading={createLoading} />}
                    ...
                    {currentSteps === 0 ?
                        ...
                        : currentSteps === 1 ?
                            <>
                                ...
                                <div>
                                    <button disabled={createLoading} className={createLoading ? Style.SubmitBtnDisable : Style.SubmitBtn} onClick={() => editId ? setEditWorkOrder(true) : getStepThreeData()}>{createLoading ? <Spin /> : editId ? "Update Alert" : "Create"}</button>
                                </div>
                            </> :
                            ""
                    }
                ...
                <Modal
                    ...
                    footer={<>
                        <div className={Style.editPersonalModalFooter}>
                            ...
                            <button style={{ background: createLoading ? 'var(--gray-60)' : 'var(--primary)' }} disabled={createLoading} onClick={() => { getStepThreeData() }} className={createLoading ? Style.editPersonalModalFooterDeleteD : Style.editPersonalModalFooterDelete}>{`Update Alert`}</button>
                        </div>
                    </>}

                >
                    <>
                        <h4 className={Style.AreYouSure}>Are you sure you want to update this Alert?</h4>
                        <p className={Style.AreYouSurePara}>You're about to update this Alert. Changes will be saved and visible to all relevant team members.</p>
                    </>
                </Modal>
                {/* confirm Edit */}
                ...
                <Modal
                    ...
                    footer={<>
                        <div className={Style.editPersonalModalFooter}>
                            ...
                            <button style={{ background: createLoading ? 'var(--gray-60)' : 'var(--primary)' }} disabled={createLoading} onClick={() => { getStepThreeDataSaveData() }} className={createLoading ? Style.editPersonalModalFooterDeleteD : Style.editPersonalModalFooterDelete}>{`Save Alert`}</button>
                        </div>
                    </>}

                >
                    <>
                        <h4 className={Style.AreYouSure}>You want to save this Alert?</h4>
                        <p className={Style.AreYouSurePara}>You're about to save this Alert. Changes will be saved and visible to you in draft section.</p>
                    </>
                </Modal>
                {/* confirm from WorkOrder  Draft */}
...
export default connect(
    mapStateToProps,
    { ...ProjectAction, ...POIAction, ...AlertAction }
)(AlertScreenCreate);
...
const BasicInformation = forwardRef(({ PoiReducer, counter, basicInfoSectionRef, ComapnyUserData, WorkOrderReducerData, messageApi, WorkOrderReducer, isValidBtn, editId, workOrderGetByIDData }) => {
            ...
            reset(
                {
                    ...
                    typeOfAlerts: workOrderGetByIDData?.alertType,
                }
            )
    ...
    const schema = yup.object().shape({
        Title: yup.string().required(),
        threatLevel: yup.string().required(),
        typeOfAlerts: yup.string().required()
    });
                            ...
                            <Col xxl={24} xl={24} lg={24} md={24} sm={24} xs={24}>
                                <div className={Style.FeildCol}>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field: { onChange, value } }) => (
                                            <Input onChange={onChange} value={value} status={errors?.Title?.message !== undefined ? 'error' : ''} placeholder='Enter Alert title' />
                                        )}
                                        name="Title"
                                    />
                                </div>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field: { onChange, value } }) => (
                                            <Select
                                                ...
                                                onChange={onChange}
                                                value={value == "" ? null : value}
                                                status={errors?.typeOfAlerts?.message !== undefined ? 'error' : ''}
                                                options={typesOfAlert}
                                            />)}
                                        name="typeOfAlerts"
                                    />
...
const POICustomization = forwardRef(({ fineEdit, setIsDraft, isDraft, alertDetail, workOrderLinkData, PoiReducer, counter, taskAndLocationRef, messageApi, editId, workOrderGetByIDData, createLoading }) => {
    const navigate = useNavigate();
    const now = new Date(Date.now());
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, '0');
    ...
    const schema = yup.object().shape({
        ...
        preNotification: yup
            ...
            .min(5, 'Value must be 5 or greater')
            .notRequired(),
    });
    const localData = JSON.parse(localStorage.getItem(localStoreKey) || "{}");
                ...
                <Row gutter={16} style={{ width: '100%' }}>
                    ...
                    <Col xxl={24} xl={24} lg={24} md={24} sm={24} xs={24} >
                        <Row gutter={16}>
                            ...
                            <Col xxl={12} xl={12} lg={12} md={24} sm={24} xs={12}>
                                <ExtraData name={"Alert"} createLoading={createLoading} extraDataList={extraDataList} setExtraDataList={setExtraDataList} localStoreKey={localStoreKey} messageApi={messageApi} />
                            </Col>
                        </Row>
                    </Col>
                    <Col xxl={24} xl={24} lg={24} md={24} sm={24} xs={24}>
                            ...
                            <Col xxl={12} xl={12} lg={12} md={24} sm={24} xs={12}>
                                <div className={Style.TaskFeild} style={{ marginTop: 16 }}>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field: { onChange, value } }) => (
                                            <>
                                                <InputNumber disabled={createLoading} max={999} maxLength={3} onChange={onChange} value={value} status={errors?.preNotification?.message !== undefined ? 'error' : ''} placeholder='Enter Pre-Notification Time' />
                                                {/* <p style={{ marginTop: 4, fontSize: 12, marginBottom: 0 }}>Hellp</p> */}
                                            </>
                                        )}
