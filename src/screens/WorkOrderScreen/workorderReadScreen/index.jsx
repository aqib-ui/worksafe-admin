...
import { AWSUploadModule, AWSUploadModuleFilter } from '../../../component/AWSUploadModule';




const WorkorderScreenRead = ({ workOrderGetDoc, GetMyAssignedWorkOrder, GetMyWorkOrder, ArchiveWorkOrder, ApproveWorkOrder, DeclineWorkOrder, PermissionReducer, WorkOrderReducer, GetWorkSite, GetCompanyUser, WorkOrderGetById, GetAdminWorkSite, CompleteWorkOrder }) => {
    ...
    useEffect(() => {
        ...
        const runEffect = async () => {
            ...
            // ===== SHAPES =====
            const polygon = workOrderGetByIDData?.polygon;

            if (polygon?.type === "Circle") {
                setSelectShape(2);
                setCircleRadius(Number(polygon.radius?.toFixed()));
                setCircleSafety(Number(polygon.safetyZone?.toFixed()));
                setCircleElevation(Number(polygon.altitude?.toFixed()));
                ...
                timeoutId = setTimeout(() => {
                    ...
                    drawWithRadiusBounds(position, Number(polygon.radius?.toFixed()));
                    setMapLoader(false);
                }, 2000);
            }
                ...
                timeoutId = setTimeout(() => {
                    ...
                    setCustomAreaElevation(Number(polygon.altitude?.toFixed()));
                    drawPolyLinePolyGoneBond(polygon.locations);
                    setPolygonPoint(
                        polygon.locations?.map(([lat, lng]) => ({
                            lat: Number(lat),
                            lng: Number(lng),
                        })) || []
                    );
                ...
                timeoutId = setTimeout(() => {
                    ...
                    setPolylineSafety(Number(polygon.safetyZone?.toFixed()));
                    setPolylineElevation(Number(polygon.altitude?.toFixed()));
                    drawPolyLinePolyGoneBond(polygon.locations);
                    setPointsPolyLine(
                        polygon.locations?.map(([lat, lng]) => ({
                            lat: Number(lat),
    ...
    const menuItems = () => {
        ...
        // Assigned by me
        if (workOrderGetByIDData?.created_by?._id === UserID) {
            if (workOrderGetByIDData?.status === "completed") {
                return [
                    {
                        key: "approve",
                        label: "Approve Work Order",
                        icon: <FaCheck size={18} color="green" />,
                        onClick: () => ApproveWorkOrder(workOrderGetByIDData?._id),
                    },
                    {
                        key: "decline",
                        label: "Decline Work Order",
                        icon: <IoMdClose size={18} />,
                        onClick: () => DeclineWorkOrder(workOrderGetByIDData?._id),
                    },
                    {
                        key: "archive",
                        ...
                        disabled: !canDelete,
                        onClick: () => canDelete && setDeleteExtra(true),
                    },
                ];
            }
            if (workOrderGetByIDData?.status === "approved") {
                return [
                    {
                        key: "archive",
                        label: "Archive Work Order",
                ...
                >
                    ...
                    <div style={{ paddingTop: 15 }}>
                        <div className={Style.TaskFeild}>
                            ...
                            <DatePicker onChange={(e) => setCurrentDate(e)} value={currentDate ? dayjs(currentDate) : dayjs(getCurrentDate(), dateFormat)} format={dateFormat} suffixIcon={<img src={calendarDatePicker} style={{ height: "24px" }} />} showTime={{ format: 'hh:mm A', use12Hours: true, showSecond: false, }} minDate={dayjs(formattedDate, dateFormat2)} placeholder='Select date & time' />
                        </div>
                    </div>
...
export default connect(mapStateToProps, WorkOrderAction)(WorkorderScreenRead);
